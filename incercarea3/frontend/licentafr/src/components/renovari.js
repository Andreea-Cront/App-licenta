...
const Renovari = () => {
  const [renovariServices, setRenovariServices] = useState([]);
  ...
  useEffect(() => {
    const fetchRenovariServices = async () => {
      try {
        const response = await axios.get('http://localhost:8081/renovari-services');
        setRenovariServices(response.data);
      } catch (error) {
        console.error('Error fetching auto services:', error);
      }
    };

    fetchRenovariServices();
  }, []);
  ...
  const renderStars = (rating) => {
    return (
      <div className='rating-stars'>
        {[...Array(5)].map((star, i) => (
          <span key={i} className={`star ${i < rating ? 'filled-star' : ''}`}>★</span>
        ))}
      </div>
    );
  };
  return (
    <div>
      <h1 className='titlul'>Servicii Renovări</h1>
      ...
      <div className='auto-container'>
        <br></br>
        {renovariServices.map((service, index) => (
          <div className='auto-service' key={index}>
            ...
            <button onClick={() => handleAddToCart(service)}>Adaugă în coș  
            <AddShoppingCartIcon/>
            </button>
          </div>
        ))}
